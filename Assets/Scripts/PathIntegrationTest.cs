...
public class PathIntegrationTest : TaskBase {
        ...
        if (m_TrialIndex < this.GetComponent<TrianglePathController>().NumTriangles * m_NumTrials)
        {
            switch (TaskState)
            {
                ...
                case ETaskState.Execution:
                    ...
                    if (m_Delay > 2.0f && VRInput.GetUp(VRInputButton.Trigger))
                    {
    ...
    private void SetData()
    {
        ...
        float signedAngle = Vector3.SignedAngle(dirPath, dirExec, this.transform.up);

        float offset = Vector3.Distance(targetPos, pos);
        float travelDist = Vector3.Distance(pos, transform.Find("PointB").position);
